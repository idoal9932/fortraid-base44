...
export default function CheckupHistoryDialog({ siteName, checkups, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-card w-full max-w-md rounded-2xl p-6 max-h-[80vh] flex flex-col gap-3"
                ...
                <CardContent className="px-4 py-3 flex items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {c.check_date && !isNaN(new Date(c.check_date)) ? format(new Date(c.check_date), "dd/MM/yyyy HH:mm") : ""}
                    </p>
                  </div>
