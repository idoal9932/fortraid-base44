...
import { Package, Plus, Minus, Trash2, Check } from "lucide-react";
...
export default function InventoryTable({ items, isLoading, selectedCategory, setSelectedCategory }) {
  ...
  if (isLoading) {
    return <Card><CardContent className="p-4"><div className="h-64 animate-pulse bg-muted rounded" /></CardContent></Card>;
  }
                ...
                <tr key={item.id} className={`border-b transition-colors ${getRowColor(item)} ${confirmingItem === item.id ? "animate-pulse" : ""} hover:bg-accent/50`}>
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center w-6 h-6 rounded border transition-all ${
                        item.min_threshold && item.quantity < item.min_threshold
                          ? "cursor-pointer border-primary bg-primary/10 hover:bg-primary/20"
                  ...
                  <td className="px-3 py-2 text-center">
                    {editingCell?.id === item.id && editingCell.field === "quantity" ? (
                      <Input
                        ...
                        onBlur={() => updateItemMutation.mutate({ id: item.id, data: { quantity: parseInt(editValue) || 0 } })}
                        onKeyDown={(e) => e.key === "Enter" && updateItemMutation.mutate({ id: item.id, data: { quantity: parseInt(editValue) || 0 } })}
                        autoFocus className="h-8 w-16 text-center" inputMode="numeric"
                      />
                    ) : (
                      <span onClick={() => { setEditingCell({ id: item.id, field: "quantity" }); setEditValue(item.quantity.toString()); }}
                  ...
                  <td className="px-3 py-2 text-center">
                    {editingCell?.id === item.id && editingCell.field === "min_threshold" ? (
                      <Input
                        type="number" min="0" value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => updateItemMutation.mutate({ id: item.id, data: { min_threshold: parseInt(editValue) || 0 } })}
                        onKeyDown={(e) => e.key === "Enter" && updateItemMutation.mutate({ id: item.id, data: { min_threshold: parseInt(editValue) || 0 } })}
                        autoFocus className="h-8 w-16 text-center" inputMode="numeric"
                      />
                    ) : (
                      <span onClick={() => { setEditingCell({ id: item.id, field: "min_threshold" }); setEditValue((item.min_threshold || 0).toString()); }}
                  ...
                  <td className="px-3 py-2 text-xs">
                    {editingCell?.id === item.id && editingCell.field === "expiry_date" ? (
                      <Input type="date" value={editValue} onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => updateItemMutation.mutate({ id: item.id, data: { expiry_date: editValue || null } })}
                        autoFocus className="h-8" />
                    ) : item.expiry_date ? (
                      <span onClick={() => { setEditingCell({ id: item.id, field: "expiry_date" }); setEditValue(item.expiry_date); }}
                        className="cursor-pointer hover:bg-primary/10 px-2 py-1 rounded block">
                        {format(new Date(item.expiry_date), "dd/MM/yyyy")}
                      </span>
                    ) : (
                      <span onClick={() => { setEditingCell({ id: item.id, field: "expiry_date" }); setEditValue(""); }}
                        className="cursor-pointer text-muted-foreground hover:bg-primary/10 px-2 py-1 rounded block">—</span>
                    )}
                  </td>
                  <td className="px-3 py-2 text-xs max-w-xs">
                    {editingCell?.id === item.id && editingCell.field === "notes" ? (
                      <Input type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => updateItemMutation.mutate({ id: item.id, data: { notes: editValue } })}
                        autoFocus className="h-8" />
                    ) : (
                      <span onClick={() => { setEditingCell({ id: item.id, field: "notes" }); setEditValue(item.notes || ""); }}
                  ...
                  <td className="px-3 py-2 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <Button size="icon" variant="ghost" className="h-7 w-7"
                        onClick={() => updateItemMutation.mutate({ id: item.id, data: { quantity: item.quantity + 1 } })}>
