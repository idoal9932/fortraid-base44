...
export const AuthProvider = ({ children }) => {
  ...
  const [viewAsRole, setViewAsRole] = useState(null); // זמני בלבד — לא נשמר ב-DB
  ...
  const checkUserAuth = async () => {
    try {
      ...
      // Load extended user data from User entity
      try {
      ...
      // If admin user, automatically set viewAsRole to "paramedic"
      if (currentUser?.email === 'idoal9932@gmail.com') {
        setViewAsRole('paramedic');
        setNeedsOnboarding(false);
      } else if (!currentUser?.custom_role) {
        // User exists but doesn't have custom_role — needs onboarding
